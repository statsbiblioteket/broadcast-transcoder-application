...
func_jobsummary()
{
    local numjobs pid transcode_cmd dashh host dashj queuefile
...
print_usage()
{
    echo "Usage: $(basename $0) start|stop|status [-h <host> -n <number of jobs> -j <jobfile> -v]"
    echo
...
case $mode in
	...
	if [ $currentjobs -lt $numjobs ]; then
	    ...
	    echo "$currentjobs jobs already running, not starting any more"
	    exit 5
	fi
	echo "Starting $startjobs jobs on $host"
